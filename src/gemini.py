...
def get_gemini_response(user_id, text=None, image_bytes=None):
    ...
    if (text and any(char > '\u3040' for char in text)) or image_bytes:
        ...
        # CLEAN, UNFORMATTED text with predictable headers.
        prompt_parts = [
            ...
            "\n--- YOUR TASK ---\n",
            "Provide a detailed analysis of the sentence. Organize your response with these exact headers, each on a new line:",
            "Extracted Japanese Text",
            "English Translation",
            "Vocabulary Breakdown",
            "Grammar Analysis",
            "\n--- RESPONSE FORMATTING ---\n",
            "Provide the response as plain text only. Do not use any Markdown formatting like '*', '_', '`', or '#'. I will format it later.",
            "Under 'Vocabulary Breakdown', list each vocabulary word and its details on a new line.",
            "Under 'Grammar Analysis', provide a detailed explanation. You can use numbers (e.g., '1.', '2.') or asterisks ('*') for bullet points if needed.",
        ]
        if text:
